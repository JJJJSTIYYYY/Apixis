...
`await Logger.start()` 启动后台刷新；缓存条数达到 `LOG_BUFFER_SIZE` 或缓存大小达到原有阈值，都会触发刷新，因此短日志也能在 FIFO 达到上限时写入文件。`await Logger.flush()` 将当前保留的记录按 logger 名称写入各自文件并清空缓存；`await Logger.stop()` 停止已启动的刷新任务并写出剩余记录。尚未达到任一阈值时，可通过 `flush()` 或 `stop()` 写出记录。已被 FIFO 淘汰的记录不会再写入文件。
